...
import (
	...
	hn "github.com/raflynagachi/go-rest-api-starter/internal/order/handler/definition"
...
func newRouter(cfg *config.Config, log *logger.Logger, hn hn.APIHandler, db *sqlx.DB) *httprouter.Router {
	...
	router.GET("/orders", hn.GetOrders)
	router.GET("/orders/:id", hn.GetOrderByID)
	router.POST("/orders", jwtAuth(hn.CreateOrder))
